...
#         self.targets = None
#
#     def _check_columns(self, df: pl.DataFrame):
#         columns = df.columns
...
#
#     def _add_padding(self, expr: pl.Expr) -> pl.Expr:
#         seq_len = expr.list.len().cast(pl.Int32)
#         padding = pl.lit(0).repeat_by(self.max_seq_len - seq_len)
#
#         return pl.concat_list(expr, padding)
#
#     def _create_mask(self, expr: pl.Expr) -> pl.Expr:
#         seq = pl.lit(0).repeat_by(expr)
#         mask = pl.lit(1).repeat_by(self.max_seq_len - expr)
#         seq_mask = pl.concat_list(seq, mask)
#         return (
#             pl.when(seq_mask == pl.lit(1).repeat_by(self.max_seq_len))
#             .then(pl.lit(0).repeat_by(self.max_seq_len))
#             .otherwise(seq_mask)
#         )
#
#     def _sort_dataframe(self, data: pl.DataFrame) -> Tuple[pl.DataFrame, pl.DataFrame]:
#         data = data.with_columns(
#             user_rn=pl.col(self.user_id)
#             .cum_count()
#             .over(partition_by=[self.user_id], order_by=self.order_by)
#         ).sort([self.user_id, "user_rn"])
#
#         if self.partition_by is not None:
#             data_output = data.select(
#                 pl.col(self.user_id),
#                 pl.col(self.item_id),
#                 pl.col("user_rn"),
#                 pl.col(self.partition_by),
#             )
#         else:
#             data_output = data.select(
#                 pl.col(self.user_id), pl.col(self.item_id), pl.col("user_rn")
#             )
#
...
#             df_seq = data.rolling(
#                 index_column="user_rn",
#                 group_by=[self.user_id],
#                 period=f"{self.max_seq_len}i",
#                 closed="right",
#             ).agg(pl.col(col_name).alias(col_name))
#
#             if idx == 0:
#                 df_seq = (
#                     df_seq.with_columns(
#                         seq_len=pl.col(col_name).list.len().cast(pl.Int32)
#                     )
#                     .with_columns(mask=pl.col("seq_len").pipe(self._create_mask))
#                     .with_columns(pl.col(col_name).pipe(self._add_padding))
#                 )
#
#             else:
#                 df_seq = df_seq.with_columns(pl.col(col_name).pipe(self._add_padding))
#
#             data_output = data_output.join(df_seq, on=[self.user_id, "user_rn"])
#
#             if output_targets is not None:
#                 if col_name in output_targets:
#                     target = f"t_{col_name}"
#                     self.targets.append(target)
#                     data_output = data_output.with_columns(
#                         pl.col(col_name)
#                         .shift(-1)
#                         .over([pl.col(self.user_id)])
#                         .alias(target)
#                     )
#
#         else:
...
#
#         if self.partition_by is not None:
#             columns = columns + [self.partition_by]
#
#         data_output = data_output.select(columns).sort([self.user_id, "user_rn"])
#
#         return data_output
